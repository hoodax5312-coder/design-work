...
目录先生成到同一父目录下的临时名称，manifest 和所有选择文件验证完成后再原子 rename 为最终 `.design-work-package`。这避免尚未验证的 ZIP64、超大视频、ZIP bomb 和解压路径穿越问题。压缩封装作为后续兼容层，只有在流式、ZIP64、许可证和安全测试通过后才能加入。
...
- 取消导出只留下可识别并可清理的临时目录，不留下可误认为完成的 `.design-work-package`。
...
- 完整产品文档明确要求本地服务；`Design Work-standalone.html` 标注为交互演示，不宣称具备文件系统和 SQLite 能力。
...
- 评审对象：Design Work 统一 AI 资产与知识管理平台；依据本实施计划、产品规格和当前实现。
...
价值锚点声明
────────────────────────────────────────
产品名称：Design Work 统一 AI 资产与知识管理平台
...
| 竞品对标 | 7 | 7 | Eagle 已把标签、文件夹、智能文件夹、重复扫描和快速检索作为成熟基本盘；Adobe Bridge 也提供集合、元数据与高级筛选。因此 Design Work 不应以“能分类”定位自己，应以不复制文件下的 AI 生成上下文与跨资产关系追溯取胜。参见评审报告的引用。 |
...
竞品依据：Eagle 已提供标签、文件夹、智能文件夹、重复扫描与快速检索，说明这些是设计资产管理的基线能力；Adobe Bridge 也以集合、元数据、过滤与检索组织多类型创意资产。Design Work 的差异化必须集中在本地引用、生成元数据与跨资产关系，不应以重复的基础分类能力作为卖点。
