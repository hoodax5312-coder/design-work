    ...
    <BaseNode selected={selected} icon={ImageIcon} title="图片" width={420} showTargetHandle showSourceHandle onDelete={() => deleteNode(id)}>
      <div className="px-3 pb-3">
        ...
        {selected && <div className="nodrag nowheel mt-3 w-full rounded-lg border border-border bg-background p-2 shadow-sm">
          <Textarea aria-label="图像生成提示词" value={data.prompt || ''} onChange={(event) => updateNode(id, { prompt: event.target.value })} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') { event.preventDefault(); void generate(); } }} placeholder="描述想生成的画面，@ 引用画布素材…" variant="ghost" className="nowheel h-16 min-h-0 w-full resize-none px-1 text-xs leading-5" />
          <div className="mt-2 flex items-center gap-2 border-t border-border/70 pt-2">
            <Select aria-label="选择图像模型" value={activeOption ? activeOption.provider.id + '::' + activeOption.model : ''} onChange={(event) => selectModel(event.target.value)} disabled={!providerOptions.length} selectSize="sm" className="min-w-0 flex-1 text-xs" options={providerOptions.length ? providerOptions.map((option) => ({ value: option.provider.id + '::' + option.model, label: option.model })) : [{ value: '', label: '暂无图像模型' }]} />
            <Button type="button" variant="secondary" size="sm" onClick={() => setSettingsOpen((open) => !open)} aria-expanded={settingsOpen} className="shrink-0 text-xs">{quality === 'hd' ? '高' : '标准'} · {ratio}</Button>
