    ...
    <div className="module-workspace relative flex h-full min-w-0 flex-col bg-background text-foreground">
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-lg bg-background">
        ...
        {dropZoneOpen && (
          ...
          >
            ...
            <p className="text-sm font-semibold">点击选择文件，或将文件拖入此处</p>
            <p className="mt-1 text-xs text-muted-foreground">支持一次选择或拖入多个文件</p>
          </section>
        )}
        {selectedCount > 0 && (
            ...
            <div className="flex h-full min-h-[420px] flex-col items-center justify-center text-center">
              ...
              <p className="mt-2 max-w-sm text-xs leading-5 text-muted-foreground">
                {activeFilterCount || debouncedQuery
                  ? '调整搜索或筛选条件，原有资产不会被删除。'
                  : '导入图片、视频、PPT 或知识文件。Mboard 只引用原路径，不会重复占用空间。'}
              </p>
              {!activeFilterCount && !debouncedQuery && (
      ...
      {importDialogOpen && (
        <AssetImportDialog
          ...
          onChooseFolder={() => importAssets('folder')}
        />
      )}
      <TaskDrawer
        open={tasksOpen}
