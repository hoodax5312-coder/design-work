...
import { BookmarkPlus, Check, CheckSquare2, ImagePlus, Paperclip, Pencil, Plus, Send, Tags as TagsIcon, Trash2 } from 'lucide-react';
...
export const QuickNotes = () => {
  ...
  return (
    <main className="flex h-full min-h-0 flex-col gap-3 overflow-visible bg-transparent text-foreground md:flex-row">
      <section className="relative z-20 flex max-h-[46vh] w-full shrink-0 flex-col overflow-visible bg-transparent p-3 shadow-none md:max-h-none md:w-[min(390px,38vw)] md:min-w-[300px] sm:p-4">
      ...
      <section aria-label="小记列表" className="min-h-0 flex-1 overflow-y-auto rounded-lg bg-white p-3 dark:bg-[#121212] sm:p-4">
        <div className="mx-auto max-w-4xl">
          <div aria-label="按标签筛选小记" className="sticky top-0 z-20 mb-4 flex flex-wrap items-center gap-2 bg-white pb-3 dark:bg-[#121212]">
          ...
          {filteredNotes.map((note) => (
            <Card key={note.id} padding="lg" className="group relative border-black/[0.035] bg-white dark:border-white/[0.055] dark:bg-[#171717]">
          ...
          <div className="px-5 pb-5 pt-4">
            ...
            <div className="max-h-[280px] space-y-1 overflow-y-auto pr-1">
              {managedTags.map((tag) => <div key={tag} className="group flex min-h-10 items-center gap-2 rounded-lg px-2 hover:bg-muted/70">
                {editingTag === tag ? <Input autoFocus value={editingTagInput} onChange={(event) => setEditingTagInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') saveRenamedTag(); if (event.key === 'Escape') setEditingTag(null); }} className="h-8 flex-1 text-xs" /> : <><span className="min-w-0 flex-1 truncate text-sm">{tag}</span><span className="text-xs tabular-nums text-muted-foreground">{tagCounts.get(tag) || 0}</span></>}
                {editingTag === tag ? <><Button type="button" variant="ghost" size="sm" onClick={() => setEditingTag(null)} className="h-8 px-2 text-xs">取消</Button><Button type="button" variant="primary" size="sm" onClick={saveRenamedTag} disabled={!editingTagInput.trim() || (editingTagInput.trim().replace(/^#/, '') !== tag && managedTags.includes(editingTagInput.trim().replace(/^#/, '')))} className="h-8 px-2 text-xs">保存</Button></> : <div className="flex shrink-0 gap-1 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100"><Button type="button" variant="ghost" size="iconSm" onClick={() => startRenamingTag(tag)} aria-label={`重命名标签 ${tag}`} title="重命名" className="h-8 w-8 text-muted-foreground"><Pencil size={14} /></Button><Button type="button" variant="ghost" size="iconSm" onClick={() => deleteManagedTag(tag)} aria-label={`删除标签 ${tag}`} title="删除" className="h-8 w-8 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 size={14} /></Button></div>}
              </div>)}
