...
export const QuickNotes = ({ query = '' }: { query?: string }) => {
  ...
  const [category, setCategory] = useState(() => categoryItems[0] || ''); const [manageMode, setManageMode] = useState(false); const [selected, setSelected] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (categoryItems.includes(category)) return;
  ...
  const filtered = useMemo(() => entries.filter((entry) => entry.category === category && `${entry.name} ${entry.prompt} ${entry.category}`.toLowerCase().includes(query.trim().toLowerCase())), [entries, category, query]);
