  ...
  return (
    <main className="h-full overflow-hidden bg-transparent text-slate-950 dark:text-white">
      <div className="flex h-full min-w-0 flex-col gap-3 md:flex-row">
        ...
        <section className="min-h-0 min-w-0 flex-1 overflow-y-auto rounded-lg bg-background px-4 py-4 sm:px-6">
          <header className="mb-5 flex items-end justify-between gap-4">
            <div>
              <div className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Prompt library</div>
              <h1 className="text-2xl font-semibold tracking-[-0.035em] text-foreground">{activeCategoryData.label}</h1>
            </div>
            <p className="mb-1 shrink-0 text-sm text-slate-500 dark:text-zinc-500">{visiblePrompts.length} 条结果</p>
          </header>
          ...
          <div className="grid grid-cols-1 gap-3 2xl:grid-cols-2">
            {visiblePrompts.map((prompt) => {
              ...
              return (
                  <article key={prompt.id} className="module-card prompt-library-card group flex min-h-[240px] w-full max-w-[800px] flex-col p-4">
                  <div className="flex items-start justify-between gap-3">
                    <span className="rounded-md bg-muted px-2 py-1 text-xs font-semibold text-muted-foreground">{activeCategoryData.label}</span>
                    <span className="pt-1 text-xs font-medium tracking-[0.1em] text-slate-400 dark:text-zinc-600">{prompt.id}</span>
                  </div>
                  <h2 className="mt-4 text-lg font-semibold leading-tight tracking-[-0.025em] text-foreground">{prompt.title}</h2>
                  <p className="mt-3 text-sm leading-6 text-muted-foreground">{prompt.prompt}</p>
                  <footer className="mt-auto flex items-center justify-between gap-3 pt-5">
                    <span className="text-xs font-semibold tracking-[0.1em] text-muted-foreground">ENGLISH PROMPT</span>
                    <Button type="button" variant="secondary"
                      onClick={() => copyPrompt(prompt)}
                      aria-label={`复制「${prompt.title}」提示词`}
                      className="h-8 shrink-0 px-3 text-xs"
                    >
                      {isCopied ? <Check aria-hidden="true" size={18} /> : <Copy aria-hidden="true" size={18} />}
                      {isCopied ? '已复制' : '复制'}
                    </Button>
                  </footer>
                </article>
