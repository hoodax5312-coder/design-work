...
export const SourceCenter = ({ initialSection = 'notes' }: { initialSection?: 'notes' | 'prompts' | 'documents' }) => {
  ...
  return (
    <div className="module-workspace flex h-full min-h-0 flex-col">
      <Tabs value={activeSection} onValueChange={(value) => setActiveSection(value as typeof activeSection)} className="ui-module-toolbar mx-[40px] h-14 shrink-0 border-0 px-0 shadow-none">
        <TabsList aria-label="文档中心内容" className="h-8 gap-0 rounded-lg bg-muted p-0.5 shadow-none"><TabsTrigger value="notes" className="h-7 border-0 px-4 text-xs shadow-none data-[state=active]:bg-background data-[state=active]:shadow-sm">小记</TabsTrigger><TabsTrigger value="prompts" className="h-7 border-0 px-4 text-xs shadow-none data-[state=active]:bg-background data-[state=active]:shadow-sm">提示词库</TabsTrigger><TabsTrigger value="documents" className="h-7 border-0 px-4 text-xs shadow-none data-[state=active]:bg-background data-[state=active]:shadow-sm">文档</TabsTrigger></TabsList>
        <div className="relative ml-auto w-[240px] shrink-0">
          ...
          <Input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="搜索文档内容" placeholder={activeSection === 'documents' ? '搜索文档和文件夹…' : '搜索标题、描述和提取文字…'} className="h-8 border-0 bg-[#f3f3f1] py-0 pl-9 shadow-none focus-visible:ring-1 focus-visible:ring-black/10 dark:bg-white/[0.06] dark:focus-visible:ring-white/15" />
        </div>
      </Tabs>
      <div className="mx-[40px] mb-0 min-h-0 flex-1 overflow-visible rounded-xl bg-[#f8f8f6] p-2 dark:bg-white/[0.035]">
