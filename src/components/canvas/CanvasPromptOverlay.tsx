...
export const CanvasPromptOverlay = ({ node, selected }: { node: Node; selected: boolean }) => {
  ...
  const options = useMemo(() => providers.flatMap((provider) => getConfiguredModels(provider)
    .filter((model) => modelSupportsCategory(model, category))
  ...
  const generate = async () => {
    ...
    try {
      const result = await generateProviderImage({ ...provider, model }, { prompt: String(prompt), size: ratio === '16:9' ? '1536x1024' : ratio === '9:16' ? '1024x1536' : '1024x1024', quality });
  ...
  return (
    <NodeToolbar isVisible={selected} position={Position.Bottom} offset={14} align="center" className="!z-[80] !p-0">
      <div className="nodrag nowheel w-[392px] max-w-[min(392px,calc(100vw-32px))] overflow-visible rounded-xl border border-black/[0.08] bg-white p-3 text-foreground shadow-[0_12px_32px_rgba(17,23,19,0.14)] dark:border-white/[0.1] dark:bg-[#151515] dark:shadow-[0_12px_32px_rgba(0,0,0,0.32)]" onPointerDown={(event) => event.stopPropagation()}>
        <div className="rounded-lg bg-[#f6f6f3] px-1 dark:bg-white/[0.05]">
          ...
          <div className="flex items-center gap-1 border-t border-black/[0.06] px-1 pb-1 pt-1 dark:border-white/[0.08]">
